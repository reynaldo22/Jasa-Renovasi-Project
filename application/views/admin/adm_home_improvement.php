...
<!--four-grids here-->
      ...
      <a href="<?=base_url()?>home/add_home_improvement" class="btn btn-info">Tambah</a>
        ...
        <table id="daftar" class="table table-striped">
          ...
          <tbody>
            <?php foreach($home_improvement as $home) { ?>
            <tr>
              <td><?=$this->security->xss_clean($home->id) ?></td>
              <td><?=$this->security->xss_clean($home->nama) ?></td>
              <td><?=$this->security->xss_clean($home->gambar) ?></td>
              <td><?=$this->security->xss_clean($home->gambar) ?></td>
              <td><a href="<?=base_url()?>home/edit_home_improvement/<?php echo $home->id ?>" class="btn btn-success">Edit</a><a href="<?=base_url()?>home/delete_home_improvement/<?php echo $home->id ?>" class="btn btn-danger">Delete</a></td>
            </tr>
          <?php } ?>
          </tbody>
        </table>
